...
系统新增了 `user_behavior` 行为表，用于记录用户浏览、加购、购买三类信号。其中浏览权重为 1，加购权重为 5，购买权重为 10。购买信号仅在支付成功后写入，避免未支付订单过早影响推荐。推荐服务在运行时会同时读取行为表、购物车和历史订单数据，用于构建用户兴趣画像。

推荐算法采用“四路候选融合”的方案，而不是简单热榜排序。四路策略及其权重如下：
...
协同过滤的实现方式是基于共同购买用户的邻域查找：先找出与当前用户有共同图书交集的其他用户，再推荐这些用户购买过但当前用户尚未接触的图书。内容相似策略主要根据分类、作者、出版社做匹配，并对新上架图书给予轻度加分。

AI 智能重排并未调用外部大模型，而是使用规则特征做二次排序。其特征包括兴趣强度、时间衰减、价格带匹配、库存健康度和短期热度。例如最近 7 天行为信号权重更高，价格位于用户偏好价格区间的图书会得到额外加分，库存更健康的图书也会获得小幅提升。

由于不同策略的原始分值量纲不同，服务层会先对每一路候选做归一化，再按以下公式融合：

`normalizedScore = (candidateScore / maxScore) × strategyWeight × 100`

最后按 `bookId` 去重合并，将同一本书来自不同策略的得分累加，并输出 `score`、`strategy`、`reason` 三个解释性字段，方便前端展示“为什么推荐这本书”。

### 11.2 后台 AI 店铺数据分析设计

后台 AI 分析不是直接对数据库明细表做自由查询，而是先复用经营看板服务聚合后的 `AdminDashboardVO` 数据对象，再进行分析。输入数据主要包括店铺总览指标、近 7 天订单趋势、近 7 天营收趋势、订单状态分布、热销图书、低库存图书和最近订单等。

AI 分析服务采用“大模型优先 + 规则兜底”的双通道架构：

1. 先调用本地规则分析器生成一份稳定的兜底结果。
2. 若已配置模型，则通过 OpenAI Chat Completions 兼容协议向外部大模型发送请求。
3. 若模型返回成功且 JSON 结构完整，则使用模型结果。
4. 若模型未启用、超时、异常、HTTP 非 2xx 或返回字段不合法，则退回规则分析结果。

模型调用层没有绑定特定厂商，而是实现了 OpenAI 兼容请求格式，包含 `model`、`temperature`、`max_tokens`、`response_format` 和 `messages` 等字段。因此只要目标服务支持 OpenAI 风格接口，就可以通过修改 `base-url`、`api-key` 和 `model` 完成切换，例如 OpenAI 官方接口、通义千问兼容接口或自建兼容网关。

为了保证输出稳定，Prompt 被设计为强约束结构化输出。系统明确要求模型：

1. 只能输出合法 JSON。
2. 固定输出字段为 `summary`、`metrics`、`forecast`、`insights`。
3. `metrics` 固定 4 项，`insights` 为 3 到 5 项。
4. `level` 只能为 `success`、`warning`、`info`。
5. 不能编造数据，只能基于输入的经营 JSON 分析。

服务端收到模型响应后，还会做第二层结构校验。只有当 `summary`、`metrics`、`forecast`、`insights` 四个核心结构都解析成功时，才会认为模型输出可用。否则立即回退到规则结果，从而保证前端模板不会因为模型发挥不稳定而渲染失败。

### 11.3 规则兜底分析逻辑

本地规则分析并非固定文案，而是根据经营数据动态计算得到。核心流程如下：

1. 汇总近 7 天订单量与营收。
2. 计算客单价。
3. 将 7 天趋势拆分为前半周和后半周，估算增长率。
4. 使用增长率做保守线性外推，预测下周订单和营收。
5. 根据库存、待发货订单、爆款图书和客单价等条件生成运营建议。

其中预测因子会被限制在合理区间内，避免生成过度乐观或过度悲观的结果，从而提高后台展示的可信度与稳定性。
